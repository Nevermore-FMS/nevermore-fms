...
impl NodeQuery {
    ...
    async fn node<'ctx>(&self, ctx: &Context<'ctx>, id: ID) -> Result<Node> {
        let app = ctx.data::<ThreadSafeApplication>()?;
        let db = app.read().await.database.clone();
        let (type_name, id) = decode_id(id)?;
        match type_name.as_str() {
            "Plugin" => Ok(Node::Plugin(Plugin::get(db, id).await?)),
            "User" => Ok(Node::User(User::get(db, id).await?)),
            _ => Err(Error::new("unknown type_name")),
        }
    }
}
