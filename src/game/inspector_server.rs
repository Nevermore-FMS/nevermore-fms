...
// Modified for use in the Nevermore FMS.
...
use std::process;
use std::sync::{Arc, Mutex}; // Alias for the future `!` type.
use std::thread;
use uuid::Uuid;
...
impl InspectorServer {
    pub fn new(host: SocketAddr, name: String) -> Self {
        ...
        let thread_handle = thread::spawn(move || {
            let rt = crate::application::create_basic_runtime();
...
fn handle_ws_request(
    ...
    inspector_map: Arc<Mutex<HashMap<Uuid, InspectorInfo>>>,
...
async fn server(
    ...
    let inspector_map_ = Arc::new(Mutex::new(HashMap::<Uuid, InspectorInfo>::new()));
    ...
    select! {
      ...
      _ = deregister_inspector_handler => {},
      _ = server_handler => {},
    }
}
...
fn create_websocket_proxy(
    websocket: deno_websocket::tokio_tungstenite::WebSocketStream<hyper::upgrade::Upgraded>,
